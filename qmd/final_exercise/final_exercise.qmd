...
[Download](../workflow/workflow_exercise.qmd#exercise-2-download-data) the data sets [`vb_w` and `vb_l`](https://github.com/SeHellmann/r_tutorial/tree/main/raw_data) and [load the data](../load_data/load_data.qmd) into R. `vb_w` contains the stats of the winning team, `vb_l` the stats of the losing team.
...
[Select](../subsetting/subsetting.qmd#columns) only the columns from the data frames that are relevant to our 'research question'.
...
Calculate the mean height by team. Add the results in two [new columns](../getting_started/the_big_picture.qmd#adding-a-new-column) to the `vb` data frame, one for the losing team mean height, and one for the winning team mean height.
...
Do a paired t-test comparing the winning teams height vs the losing teams height. Again use a [`for-loop`](../loops/loops.qmd) to test for men and women separately. Save the result in a [list](../data_structures/data_structures.qmd#honorable-mention-list), and name the list elements.
