...
Remember how we used the `group_by()` command to [calculate the number of gold medals for each country](../merging/merging.qmd#before-merging)? Well, now you know enough to do something similar without using the tidyverse, by using a `for-loop`.
