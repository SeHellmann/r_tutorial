...
Inside the `if()` we define our [condition](../basics/basics.qmd#comparisons-and-logical-operators). Then, inside the `{ }` we define an operation that gets executed, if this condition is met. Inside the `else{ }` part we define an operation that gets executed if the condition in `if()` is not met.
...
Note that the look of this `ifelse()` function is a bit different from our `if else statement`, but the logic behind it is exactly the same: Here we [add the new column](../data_structures/data_structures.qmd#adding-new-columns) `n_100` which gets filled with `TRUE` and `FALSE`. If the conditional statement `medal_counts$n >= 100`, which is the first argument of the `ifelse()` function, is met, the function returns a `TRUE`, if not a `FALSE`.
