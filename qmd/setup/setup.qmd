...
The script pane is used to edit [scripts](../workflow/workflow.qmd#scripts). Scripts are the files you store your code in. You can execute a line of code by pressing `ctrl + enter` (on windows) or `command + return` (on macOS). To execute multiple lines of code at once, mark them before you press the keys. Try it yourself! Write:
...
[Plots](../plotting/plotting.qmd) you build in your R session get output in the `Plot tab`. If you call the [help function](../basics/basics.qmd#help) the documentation opens in the `Help tab`. The `Files tab` let's you mange the files in your working directory.
