...
Let's load our Olympic athletes data set into R. By looking at it's ending, we can see it is as `.rds` file, so it is R-specific, and can only be loaded into R. By taking a quick look into our [table](./load_data.qmd#Data-types) we can see we have to use the `readRDS()` function for loading `.rds` files.
