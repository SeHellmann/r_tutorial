...
Wondering what the `%>%` means? That's a [pipe operator](../basics/tidyverse.qmd#the-pipe-operator), which is used, mainly in the [`tidyverse`](../basics/tidyverse.qmd), to connect multiple function calls. This can make code a lot more readable. Here we start with the `babynames_merged` data set and then perform an operation on it, in this case filtering specific values.

## Adding a new column

Now, we want to plot the percentages of each name instead of the probability, because it looks a bit more intuitive (in my opinion). So, let's [build a new column](../getting_started/the_big_picture.qmd#adding-a-new-column) named `percentage`, which is just the `prop` column multiplied by `100`:
...
Now we can trim down our data set a bit more and [select](../subsetting/subsetting.qmd#columns-tidyverse) only the columns we are actually going to need:
...
We start with the function [`ggplot()`](../plotting/plotting.qmd#ggplot), in which we define our data and the x and y axes. This will draw our (empty) coordinate system. We have to use the [`aes()`](../plotting/plotting.qmd#aes) (aesthetics) function for everything that changes in relation to the data. For example, the exact location of each element in the plot is dependent on the x and y position deposited in the data, so we have to specify our axes inside the `aes()` function:
...
Now that we have defined our aesthetics, we can add a [`geom`-layer](../plotting/plotting.qmd#geom_). This will make use of the data we have defined in `ggplot()` and plot some bars for us:
