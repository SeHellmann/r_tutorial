...
Remember the weird `NA` rows we have encountered when [subsetting by condition](../subsetting/subsetting.qmd#conditional-filtering)? We were able to steer around that by using `filter()`, but let's take a closer look at that now.
